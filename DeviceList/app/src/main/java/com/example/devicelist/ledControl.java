...
/*
                            labelTemperatura.setText("Temperatura: "+array[0]+" C");

                            labelPote.setText("Voltaje: "+array[1]+" v");
                            labelHall.setText("Campo magnético: "+array[2]+" mT");
                            labelMotor.setText("Motor: "+array[3]+" rpm");*/

                            //Luego pote que es float
                            /*buffer = new byte[5];
                            bytes = socketInputStream.read(buffer);            //read bytes from input buffer
                            readMessage = new String(buffer, 0, bytes);
                            readMessage=readMessage.replace("\n","");
                            // Send the obtained bytes to the UI Activity via handler
                            labelPote.setText("Voltaje: "+readMessage+" v");
                            //Luego hall float
                            buffer = new byte[9];
                            bytes = socketInputStream.read(buffer);            //read bytes from input buffer
                            readMessage = new String(buffer, 0, bytes);
                            readMessage=readMessage.replace("\n","");
                            // Send the obtained bytes to the UI Activity via handler
                            labelHall.setText("Campo magnético: "+readMessage+" mT");

                            //Luego motor int
                            buffer = new byte[1];
                            bytes = socketInputStream.read(buffer);
                            readMessage = new String(buffer, 0, bytes);
                            System.out.println("Valor: "+readMessage);
                            aux+=readMessage;
                            while(!readMessage.equals("C")){
                                buffer = new byte[1];
                                bytes = socketInputStream.read(buffer);
                                readMessage = new String(buffer, 0, bytes);
                                System.out.println("Valor: "+readMessage);
                            ...
                            readMessage=aux.replace("\n","").replace(" ","").replace("C","");
                            // Send the obtained bytes to the UI Activity via handler
                            labelMotor.setText("Motor: "+readMessage+" rpm");
                            buffer = new byte[2];
                            bytes = socketInputStream.read(buffer);*/
                        } catch (IOException e) {
                            e.printStackTrace();
